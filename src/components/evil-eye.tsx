...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      if (program) {
        ;(program.uniforms as any).uResolution.value = [gl.canvas.width, gl.canvas.height, gl.canvas.width / gl.canvas.height]
      }
    }
    ...
    function update(time: number) {
      ...
      ;(program.uniforms as any).uMouse.value = [mouse.x, mouse.y]
      ;(program.uniforms as any).uTime.value = time * 0.001
